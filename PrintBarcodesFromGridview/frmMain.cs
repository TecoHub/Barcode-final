        ...
        {           
            ...
            try
            {
                if (lvPrint.CheckedItems.Count > 0)
                {
                    ...
                    {
                        st = "Select Product_Code As MRP, JanCode1 As Barcode, Product_Name As ProductName, P_PriceRate As SalesRate From Product_New Where Product_Code In (" + Condition + ") And JanCode1 In (" + Condition1 + ")";
                        con.Open();

                        for (int j = 1; j <= NoOfStickers - 1; j++)
                            st += "Union All Select Product_Code As MRP, JanCode1 As Barcode, Product_Name As ProductName, P_PriceRate As SalesRate From Product_New Where Product_Code In (" + Condition + ") And JanCode1 In (" + Condition1 + ")";
